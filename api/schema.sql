...
-- La unión es SOLO por invitación entre amigos aceptados; `join_code` es un
-- resto de la versión con códigos (se sigue llenando por el NOT NULL/UNIQUE
-- de las tablas vivas, pero no se expone al cliente).
CREATE TABLE IF NOT EXISTS shared_albums (
  id          BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  owner_id    BIGINT UNSIGNED NOT NULL,                -- dueño = pool (su collection)
  name        VARCHAR(80) NOT NULL,
  join_code   VARCHAR(24) NOT NULL,                    -- legacy, no se usa
  created_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
